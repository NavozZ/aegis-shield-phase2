...
| Threat | Description | Mitigation |
| :--- | :--- | :--- |
| **Unauthorized Cash-In/Out** | An attacker creates an operation on behalf of a victim. | Agent operations are two-step. The Agent *previews* the operation (generating a short-lived `intentToken`), and the Customer *confirms* it securely, transferring liability. |
...
*Status: Implemented as part of P08-Channels release.*
