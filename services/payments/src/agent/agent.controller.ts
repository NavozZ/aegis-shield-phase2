...
export class AgentController {
  ...
  async previewCashIn(
    ...
    return this.agentService.preview(
      { ...body, operationType: 'AGENT_CASH_IN' },
      correlationId,
    );
  }
  ...
  async previewCashOut(
    ...
    const correlationId =
      (request.headers['x-correlation-id'] as string) || crypto.randomUUID();
    return this.agentService.preview(
      { ...body, operationType: 'AGENT_CASH_OUT' },
      correlationId,
    );
  }
  ...
  async confirm(
    @Body()
    body: { agentId: string; intentToken: string; idempotencyKey: string },
    @Req() request: Request,
  ) {
    const correlationId =
      (request.headers['x-correlation-id'] as string) || crypto.randomUUID();
