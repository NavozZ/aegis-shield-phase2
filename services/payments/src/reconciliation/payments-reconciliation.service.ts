...
export class PaymentsReconciliationService {
  ...
  async run(): Promise<PaymentsReconciliationResult> {
    ...
    const [
      ...
      client.qrRedemption.count({
        where: {
          status: 'COMPLETED',
          ledgerJournalId: null,
        },
      }),
      client.agentCashOperation.count({
        where: {
          status: 'COMPLETED',
          ledgerJournalId: null,
        },
    ...
    add('COMPLETED_QR_MISSING_LEDGER_DATA', 'ERROR', qrMissingLedger);
    add('COMPLETED_AGENT_MISSING_LEDGER_DATA', 'ERROR', agentMissingLedger);
