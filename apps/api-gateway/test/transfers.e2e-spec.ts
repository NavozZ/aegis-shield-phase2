...
describe('Customer transfers through Gateway, Identity, Payments and Ledger (e2e)', () => {
  ...
  // Risk uses the same managed helper as the other services, so a Risk startup
  // failure reports its exit code and sanitised output rather than a bare
  // "exited during startup".
  let risk: ManagedService | undefined;
  ...
  beforeAll(async () => {
    ...
    await startLedger();
    // Risk starts before Payments, because Payments enforces Risk decisions on
    // the transfer path and would otherwise come up with its dependency absent.
    risk = start('Risk', 'services/risk/dist/main.js', 'services/risk');
    await waitForService(risk, `${process.env.RISK_SERVICE_URL}/health/live`);
    payments = start(
      'Payments',
  ...
  afterAll(async () => {
    ...
    await Promise.all(
      [identity, payments, ledger, risk].map((service) => stopService(service)),
    );
